...
export const create = async (dispatch, token, input) => {
  try {
    ...
    const { data } = await axiosInstance.post(
      "/api/admin/manager", { ...input, role: 'manager' },
      { headers: { Authorization: token } }
    );

    console.log("Manager add data", data);
...
export const getAll = async (dispatch, token, curPage, resultPerPage, query) => {
  let url = `/api/admin/users/?keyword=${query}&resultPerPage=${resultPerPage}&currentPage=${curPage}&role=manager`;
  try {
...
export const getDetails = async (dispatch, token, id) => {
  ...
  try {
    ...
    const { data } = await axiosInstance.get(`/api/admin/user/${id}/?role=manager`, {
      headers: { Authorization: token },
    });
...
export const updateWarehouse = async (dispatch, token, { managerId, warehouse }) => {
  try {
    dispatch({ type: "UPDATE_REQUEST" });

    await axiosInstance.put(`/api/admin/warehouse/assign`, { managerId, warehouse }, {
      headers: { Authorization: token },
    });
