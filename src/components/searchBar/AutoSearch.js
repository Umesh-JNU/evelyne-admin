...
const AutocompleteSearch = ({ onSelect, searchType, unassigned = '' }) => {
    ...
    if (searchTerm) {
      (async () => {
        try {
          ...
          switch (searchType) {
            case "warehouse":
              var { data } = await axiosInstance(`/api/warehouse/?keyword=${searchTerm}&unassigned=${unassigned}`);
            ...
            case "manager":
              ...
              console.log(data)
              setFilteredOptions(data.users.filter(user => user.warehouse === null));
              break;
            ...
            case "order":
              ...
              console.log(data)
              setFilteredOptions(data.orders.filter(order => order.transaction === null));
              break;
      ...
      />
      {
        loading ?
          <ListGroup className="autocomplete-list">
            <ListGroup.Item className='autocomplete-item'>
              <Skeleton count={3} />
            </ListGroup.Item>
          </ListGroup> :
          filteredOptions.length === 0 && searchTerm ?
            <ListGroup className="autocomplete-list">
              ...
              {['user', 'manager', 'controller'].includes(searchType) &&
                <ListGroup.Item className="autocomplete-item">
